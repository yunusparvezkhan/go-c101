...
func getNameCounts(names []string) map[rune]map[string]int {
	userMapAlpha := make(map[rune]map[string]int)
	...
	for _, name := range names {
		...
		firstChar := name[0]
		_, exists := userMapAlpha[rune(firstChar)]
		if !exists {
			userMapAlpha[rune(firstChar)] = make(map[string]int)
		}
		userMapAlpha[rune(firstChar)][name]++
	}
	return userMapAlpha
}
