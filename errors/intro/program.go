...
func sendSMSToCouple(msgToCustomer, msgToSpouse string) (float64, error) {
	// ?
	msgToCus, err := sendSMS(msgToCustomer)

	var msgToCusCost float64
	var msgToSpcCost float64

	if err == nil {
		msgToCusCost = msgToCus
	} else {
		return 0.0, err
	}

	msgToSpc, err := sendSMS(msgToSpouse)
	if err == nil {
		msgToSpcCost = msgToSpc
	} else {
		return 0.0, err
	}

	return msgToCusCost + msgToSpcCost, nil
}
